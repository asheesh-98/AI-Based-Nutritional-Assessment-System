    ...
    <DashboardLayout title="Overview" subtitle="Your personalized health dashboard">
      ...
      <div className="flex flex-col gap-8 max-w-7xl mx-auto w-full">
        ...
        <motion.div
          ...
          className="glass-card p-8 sm:p-10 relative overflow-hidden gradient-border shadow-[0_8px_32px_rgba(0,0,0,0.4)]"
        >
          ...
          <div className="relative z-10">
            <div className="inline-flex items-center gap-2 mb-4 px-3 py-1.5 rounded-full glass border border-white/10 text-xs font-semibold text-cyan-400">
            ...
            <p className="text-slate-400 text-lg max-w-2xl font-medium">Your metabolic optimization is on track. Check your latest insights below.</p>
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-2 flex flex-col h-full">
            <motion.div
              ...
              className="glass-card p-6 sm:p-8 flex-1 flex flex-col"
            >
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-xl font-bold text-white tracking-tight">Recent Assessments</h3>
                <Link to="/reports" className="text-sm font-semibold text-cyan-400 hover:text-cyan-300 transition-colors flex items-center gap-1">
              ...
              {data?.recent_predictions?.length > 0 ? (
                <div className="space-y-4 flex-1">
                  {data.recent_predictions.slice(0, 4).map((pred, idx) => (
                    <motion.div 
                      ...
                      className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-2xl glass border border-white/5 transition-all cursor-pointer"
                    >
                      <div className="flex items-center gap-4 mb-3 sm:mb-0">
                        ...
                        <div>
                          <p className="text-base font-bold text-white">{pred.deficiency || 'Routine Assessment'}</p>
                          <p className="text-xs text-slate-400 font-medium">{pred.date || 'Today, 10:45 AM'}</p>
                        </div>
                      </div>
                      <span className={`self-start sm:self-auto text-xs px-3 py-1.5 rounded-full font-bold uppercase tracking-wider ${
                ...
                <div className="text-center py-12 flex-1 flex flex-col items-center justify-center">
                  ...
                  <h4 className="text-lg font-bold text-white mb-2">No assessments yet</h4>
                  <p className="text-slate-400 mb-6 max-w-sm mx-auto">Complete your symptom tracker or upload blood reports to get a diagnostic reading.</p>
                  <Link
                    to="/prediction"
                    className="inline-flex items-center gap-2 px-6 py-3 gradient-bg text-white font-bold rounded-xl shadow-lg shadow-cyan-500/20 hover:shadow-cyan-500/40 transition-shadow"
                  >
          ...
          <div className="flex flex-col h-full">
            <motion.div
              ...
              className="glass-card p-6 sm:p-8 flex-1"
            >
              ...
              <div className="space-y-3">
                {quickActions.map((action, idx) => (
                  <motion.div key={action.path} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 + idx * 0.1 }}>
                    ...
                    >
                      <div className={`p-3 rounded-xl ${action.gradient ? 'bg-white/20' : 'bg-white/5 group-hover:bg-cyan-500/20 transition-colors'}`}>
                        <action.icon className={`w-5 h-5 ${action.gradient ? 'text-white' : 'text-slate-300 group-hover:text-cyan-400 transition-colors'}`} />
                      </div>
                      <span className={`text-base font-bold ${action.gradient ? 'text-white' : 'text-slate-300 group-hover:text-white transition-colors'}`}>
                        {action.label}
                      </span>
                      <ChevronRight className={`w-5 h-5 ml-auto ${action.gradient ? 'text-white/60' : 'text-slate-500 group-hover:text-cyan-400 transition-colors'}`} />
                    </Link>
                  </motion.div>
                ))}
              </div>
